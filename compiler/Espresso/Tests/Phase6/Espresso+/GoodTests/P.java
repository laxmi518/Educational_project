...
public class Plot {
    ...
    public Plot(Function poly, int minx, int maxx, int miny, int maxy, int delta) {
	minX = minx;
	minY = miny;
	maxX = maxx;
	maxY = maxy;
	...
	System.out.println("       " + justify(title,(plot.length+title.length())/2));
	System.out.print("       +");
        for (int x=0;x<plot.length;x++)
            System.out.print("-");
        System.out.println("-+");
        for (int y=0; y<=plot[0].length-1; y++) {
	    if ( (maxY-y)%divmark == 0 || y==0 || y==plot[0].length-1)
		...
		if (withguidelines &&  
		    ((maxY-y)%divmark == 0 || y==0 || y==plot[0].length-1) &&
		    plot[x][y] == ' ')
		    System.out.print("-");
		else
        ...
        System.out.println("-+");
	String t = "y = " + p;
	System.out.println("       " + justify(t, (t.length()+plot.length)/2));
...
public class Poly2 extends Poly {
    ...
    public String toString() {
	return a + "x^2" +  (b!=0 ? (b<0?" - " + (-1*b):" + " + b) +"x" : "") + (c!=0 ?(c<0?" - " + (-1*c): " + "+c) : "");
    } 
	...
	return a + "(x" + (root1<0 ? " + " + (-1*root1) : " - " + root1) + ")(x" +
	    (root2<0 ? " + " + (-1*root2) : " - " + root2) +")";
    }
    
    public char[][] plot(int minx, int maxx, int miny, int maxy, int delta) {
	int X = delta*(maxx-minx+1);
