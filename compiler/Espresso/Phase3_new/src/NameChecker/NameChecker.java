		...
		for (int i=0;i<lst.nchildren;i++) {
			...
			for (int j = 0; j < lst.nchildren; j++) {
				...
				if (((first.getname()).equals(second.getname())))
					...
					Error.error(second, Type.parseSignature(second.returnType().signature()) + " " + second.getname() + "(" + Type.parseSignature(second.paramSignature()) + " )");
...
//					println("everything here");
	...
	public Object visitInvocation(Invocation in) {
		...
		if (target == null | target instanceof This) {
			...
			if(find == null)
			{
				Error.error(in,"Method '" +in.methodName().toString()  + "' not found.");
			}
